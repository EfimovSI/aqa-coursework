...
public class UiTest {
    ...
    // Позитивные сценарии
    @Test
    void shouldGetBuyPage() {
        new HomePage().buy();
    }

    @Test
    void shouldGetBuyOnCreditPage() {
        new HomePage().buyOnCredit();
    }
    ...
    void shouldPayWithApprovedAndCvvZeroesSuccessfully() {
        var info = new Info(getApprovedCardNumber(), getValidMonth(), getValidYear(), getValidOwner(), getCvcFromZeroes());
        var buyPage = new HomePage().buy();
        buyPage.sendFilledForm(info);
        buyPage.successfullPayment();
        assertEquals("APPROVED", DbUtils.getPaymentStatus());
    ...
    void shouldNotSendWithDashedName() {
        ...
        var buyPage = new HomePage().buy();
        buyPage.sendFilledForm(info);
        assertEquals("Kuzma PetrovVodkin", buyPage.getOwnerField());
    }
