...
В качестве тестируемой страницы, для которой разработан план автоматизации, выбрана страница оплаты Payment Gate
...
3. Успешная обработка платежа при вводе валидных данных. **Ожидаемый результат:** появление в браузере информационного сообщения "Успешно. Операция одобрена банком."; в базе данных в таблице payment_entity значение status:APPROVED
4. Успешная обработка платежа при вводе валидных данных и вводе в поле "CVC/CVV" нолей (000). **Ожидаемый результат:** появление в браузере информационного сообщения "Успешно. Операция одобрена банком."; в базе данных в таблице payment_entity значение status:APPROVED
5. Отклонение платежа при вводе данных с "Declined" номером карты. **Ожидаемый результат:** появление в браузере информационного сообщения "Ошибка! Банк отказал в проведении операции."; в базе данных в таблице payment_entity значение status:DECLINED
...
2. Заполнение поля отсутствующим в базе данных номером, например 1234 1234 1234 1234. **Ожидаемый результат:** появление в браузере информационного сообщения "Ошибка! Банк отказал в проведении операции."; в базе данных в таблице payment_entity значение status:DECLINED
3. Заполнение поля номером менее 16 цифр, например 1234 1234 1234 123. **Ожидаемый результат:** появление под тестируемым полем надписи "Неверный формат"
4. Заполнение поля номером более 16 цифр, например 1234 1234 1234 1234 123. **Ожидаемый результат:** ограничение ввода большего числа цифр
5. Заполнение поля нолями (0000 0000 0000 0000). **Ожидаемый результат:** появление в браузере информационного сообщения "Ошибка! Банк отказал в проведении операции."; в базе данных в таблице payment_entity значение status:DECLINED
6. Заполнение поля специальными символами, например #!=* ?%&! $*#! :-@+. **Ожидаемый результат:** запрет ввода специальных символов
7. Заполнение поля буквами, например "abcd efgh ijkl mnop". **Ожидаемый результат:** запрет ввода букв
##### Поле "Месяц"
1. Незаполненное поле. **Ожидаемый результат:** появление под полем надписи "Поле обязательно для заполнения"
...
## План сдачи работ
1. Написание плана тестирования - 31.05.2022
2. Написание автотестов - от 3 до 6 дней после согласования плана тестирования - 03.06.2022 - 06.06.2022
3. Написание отчетов о тестировании и автоматизации - 1 день после написания автотестов - 04.05.2022 - 07.06.2022
